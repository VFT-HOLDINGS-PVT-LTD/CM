...
class Festival_Advance extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['data_loan'] = $this->Db_model->getData('Fest_ID,Festival_Name', 'tbl_festival_types');
    ...
    public function insert_data()
    {
        ...
        $HasRow = $this->Db_model->getfilteredData("select count(EmpNo) as HasRow from tbl_festival_advance where EmpNo=$Emp and Fest_ID=$loan_id and Is_Settled=0");
            ...
            for ($i = 0; $i < $Count; $i++) {
                $data = array(
                    array(
                        ...
                        'Festival_advance_amount' => $amount,
                        //                    'Request_Date' => $year,
                        'Festival_advance_date' => $timestamp,
                        'Month' => $month,
                        'Year' => $year,
                        'NoOfInst' => $No_of_Inst,
                        'Rate' => $Rate,
                ...
                $this->db->insert_batch('tbl_festival_advance', $data);
                $this->session->set_flashdata('success_message', 'New Festival Advance added successfully');
            }
        }
        redirect('/Pay/Festival_Advance/');
    }
    ...
    public function getLoans()
    {
        ...
        if (($this->input->post("cmb_year"))) {
            if ($filter == '') {
                $filter = " where  tbl_festival_advance.year =$year";
            } else {
                $filter .= " AND  tbl_festival_advance.year =$year";
            }
        }
        ...
        if (($this->input->post("cmb_loan_type"))) {
            if ($filter == null) {
                $filter = " where tbl_festival_types.Fest_ID  ='$loan_type'";
            } else {
                $filter .= " AND tbl_festival_types.Fest_ID  ='$loan_type'";
            }
        }
        if (($this->input->post("cmb_year"))) {
            if ($filter == null) {
        ...
        if (($this->input->post("cmb_grp"))) {
            if ($filter == null) {
                $filter = " where grp.Grp_ID  ='$cmb_grp'";
            } else {
        ...
        $data['data_set'] = $this->Db_model->getfilteredData("SELECT * FROM tbl_festival_advance INNER JOIN tbl_festival_types ON tbl_festival_advance.Fest_ID = tbl_festival_types.Fest_ID 
INNER JOIN 
 tbl_empmaster ON tbl_empmaster.EmpNo = tbl_festival_advance.EmpNo
        INNER JOIN
    tbl_designations dsg ON dsg.Des_ID = tbl_empmaster.Des_ID
        LEFT JOIN
    tbl_departments dep ON dep.Dep_id = tbl_empmaster.Dep_id 
    INNER JOIN
        tbl_emp_group grp ON grp.Grp_ID = tbl_empmaster.Grp_ID
                                                                    {$filter}");

        //        var_dump($data);die;

        $this->load->view('Payroll/Festival_Advance/search_data', $data);
    }
