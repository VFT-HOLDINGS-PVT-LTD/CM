...
class Payroll_Edit extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $currentMonth = date('Y');
        // date('M');
        $data['data_set'] = $this->Db_model->getfilteredData("SELECT * FROM tbl_salary INNER JOIN tbl_empmaster ON tbl_salary.EmpNo = tbl_empmaster.EmpNo WHERE tbl_salary.Year = '" . $currentMonth . "' AND tbl_empmaster.EmpNo != '00009000' ");
        $currentMonth2 = date('F');
        $data['months'] = $currentMonth2;
        $this->load->view('Payroll/Payroll_Edit/index', $data);
    }
    ...
    public function edit_data()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $dataArray = array(
                ...
                'Performance' => $data['rowData'][7],
                'Attendances' => $data['rowData'][8],
                'Transport' => $data['rowData'][9],
                'Fuel' => $data['rowData'][10],
                'Traveling' => $data['rowData'][11],
                'SPAllowance' => $data['rowData'][12],
                'Gross_pay' => $data['rowData'][13],
                'Late_deduction' => $data['rowData'][14],
                'Ed_deduction' => $data['rowData'][15],
                'Salary_advance' => $data['rowData'][16],
                'no_pay_deduction' => $data['rowData'][17],
                'EPF_Worker_Amount' => $data['rowData'][19],
                'tot_deduction' => $data['rowData'][18],
                'D_Salary' => $data['rowData'][20],
                'EPF_Employee_Amount' => $data['rowData'][21],
                'ETF_Amount' => $data['rowData'][22],
                'Net_salary' => $data['rowData'][20],
                'Edited' => 0,
            );
    ...
    public function Payroll_Search()
    {
        ...
        if (!empty($empNo)) {
            $filter .= (empty($filter) ? " WHERE " : " AND ") . "tbl_salary.EmpNo = '$empNo'";
        }
        if (!empty($empName)) {
        ...
        $sql = "SELECT * FROM tbl_salary 
                ...
                $filter AND tbl_empmaster.EmpNo != '00009000' AND tbl_salary.Approved = '1' ";
