...
class Report_Leave extends CI_Controller {
 ...
 $data['data_cmp'] = $this->Db_model->getData('Cmp_ID,Company_Name', 'tbl_companyprofile');
        ...
        if (($this->input->post("txt_from_date")) && ($this->input->post("txt_to_date"))) {
            if ($filter == '') {
                $filter = " where  tbl_leave_entry.Leave_Date between '$from_date' and '$to_date' AND tbl_empmaster.Status = '1'";
            } else {
                $filter = " AND  tbl_leave_entry.Leave_Date between '$from_date' and '$to_date' AND tbl_empmaster.Status = '1'";
            }
        }
