...
class Report_Attendance_ATT_Sum extends CI_Controller {
    ...
    public function Attendance_Report_By_Cat() {
        ...
        if (($this->input->post("txt_from_date")) && ($this->input->post("txt_to_date"))) {
            if ($filter == '') {
                $filter = " where  ir.FDate between '$from_date' and '$to_date' AND ir.DayStatus != 'AB' AND Emp.Status = '1' ";
            } else {
                $filter .= " AND  ir.FDate between '$from_date' and '$to_date' AND ir.DayStatus != 'AB' AND Emp.Status = '1'";
            }
        }
